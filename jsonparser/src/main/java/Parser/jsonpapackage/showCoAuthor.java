...
public class showCoAuthor {
	...
	{ 
		String line;
		String filePath ="F:\\Thesislogs\\Survey logs\\showCoAuthorshipSeen.txt";
		...
		try{
			...
			while((line=srcReader.readLine())!=null){
				...
				if(((JSONObject) jsnobj).get("authorId")!=null){
					...
					String limit= querysplit[1].substring(5, querysplit[1].length());
					//System.out.println(Query);
					//String Query="MATCH (n:author)-[:COAUTHOR_OF]-(m:author) WHERE n.AUTHOR_ID=\""+ authorId+"\" RETURN count(m)";
					String Query="MATCH (n:author)-[:COAUTHOR_OF]-(m:author) WHERE n.AUTHOR_ID=\""+ authorId+"\" RETURN m LIMIT "+limit;
					writeToFile(Query);
				}
			}
		}catch(Exception e){
	 ...
	 static void writeToFile(String query){
		 StringBuilder sb;
			...
			if (UpstreamFileName == null) {
				UpstreamFileName = "F:\\Thesislogs\\ParsedQueryFiles\\parsedneo4jQueryForShowCoAuthoredSeen.csv"; //dblp+citation_0
				try {
