...
public class journalParser {
	...
	{ 
		String line;
		String filePath ="F:\\Thesislogs\\Survey logs\\ShowJournalsQ1.txt";
		...
		try{
			...
			while((line=srcReader.readLine())!=null){
				...
				if(((JSONObject) jsnobj).get("PaperId")!=null){
					...
					String Query="MATCH (n:paper)-[:PUBLISHED_BY]->(m:journal) WHERE n.PAPER_ID=\""+ paperId+"\" RETURN m";
					writeToFile(Query);
				}
			}
		}catch(Exception e){
	 ...
	 static void writeToFile(String query){
		 StringBuilder sb;
			...
			if (UpstreamFileName == null) {
				UpstreamFileName = "F:\\Thesislogs\\ParsedQueryFiles\\parsedneo4jQueryForShowJournal.csv"; //dblp+citation_0
				try {
